...
#  2 Roblox clones per device - timed hopping - links from github
#  repo: github.com/lucivaantarez/rfhop
#  v1.2 - ASCII frame, auto-fit, hard refresh, Ctrl-C stops,
#         optional Termux API (wake-lock / notification / toast)
# ============================================================
VERSION="1.2.0"
...
CLONE1_PKG=""; CLONE1_FILE="links01.txt"
CLONE2_PKG=""; CLONE2_FILE="links02.txt"
...
save_config(){ { echo "CLONE1_PKG=\"$CLONE1_PKG\""; echo "CLONE1_FILE=\"$CLONE1_FILE\""
  echo "CLONE2_PKG=\"$CLONE2_PKG\""; echo "CLONE2_FILE=\"$CLONE2_FILE\""
  echo "SETTLE=$SETTLE"; echo "HOLD=$HOLD"; echo "STAGGER=$STAGGER"; echo "FORCESTOP=$FORCESTOP"
  echo "WAKELOCK=$WAKELOCK"; echo "TERMUXAPI=$TERMUXAPI"
  echo "START_TMPL='$START_TMPL'"; } > "$CFG"; }

# ---- termux api layer (optional, graceful no-op if missing) ----
...
vibe(){   ((TERMUXAPI==1)) && _have termux-vibrate && termux-vibrate -d "${1:-200}" >/dev/null 2>&1; }
...
# ---- row primitives (all ASCII, computed from INNER) ----
prule(){ printf "%s%s%s\n" "$PINK" "$RULE" "$R"; }
pempty(){ printf "%s%s%s\n" "$PINK" "$EMPTY" "$R"; }
...
titlerow(){ local stat="$1" col="$2" lft=" RFHOP" rgt="$1 " gap; gap=$((INNER-${#lft}-${#rgt})); ((gap<1))&&gap=1
  printf "%s|%s%s%s%s%*s%s%s%s%s|%s\n" "$PINK" "$R" "$CYAN" "$lft" "$R" "$gap" "" "$col" "$rgt" "$R" "$PINK" "$R"; }
pause_enter(){ printf "  ${PINK}press enter${R} "; read -r _; }
short_pkg(){ printf '%s' "${1##*.}"; }

# ---- links ----
fetch_file(){ curl -fsSL "$REPO_RAW/$1" -o "$DIR/$1.tmp" 2>/dev/null && mv "$DIR/$1.tmp" "$DIR/$1"; }
load_into(){ local -n arr="$1"; arr=(); [ -f "$DIR/$2" ] || fetch_file "$2"; [ -f "$DIR/$2" ] || return
  local l; while IFS= read -r l || [ -n "$l" ]; do l="${l%$'\r'}"; l="${l#"${l%%[![:space:]]*}"}"; l="${l%"${l##*[![:space:]]}"}"
    [ -z "$l" ] && continue; [ "${l:0:1}" = "#" ] && continue; arr+=("$l"); done < "$DIR/$2"; }

# ---- launch (rooted): optional force-stop, then start via template ----
...
home_screen(){ fit; local p1 p2; p1=$(short_pkg "$CLONE1_PKG"); [ -z "$p1" ]&&p1="(unset)"; p2=$(short_pkg "$CLONE2_PKG"); [ -z "$p2" ]&&p2="(unset)"
  ...
  kv "CLONE 1" "$p1 / $CLONE1_FILE"; kv "CLONE 2" "$p2 / $CLONE2_FILE"; kv "TIMING" "${SETTLE}s + ${HOLD}s"; prule
...
settings_screen(){ fit
  ...
  menurow "1" "C1 PKG  $sp1"; menurow "2" "C1 FILE $CLONE1_FILE"; menurow "3" "C2 PKG  $sp2"; menurow "4" "C2 FILE $CLONE2_FILE"
  menurow "5" "LOAD    ${SETTLE}s"; menurow "6" "HOLD    ${HOLD}s"; menurow "7" "STAGGER $stg"; menurow "8" "LAUNCH TEMPLATE"
  menurow "9" "WAKELOCK $wl"; menurow "A" "TERMUX API $api"
...
test_join(){ load_into TLINKS "$CLONE1_FILE"; fit; clr
  if [ -z "$CLONE1_PKG" ] || [ ${#TLINKS[@]} -eq 0 ]; then printf "  ${YEL}set clone 1 pkg + file first${R}\n"; pause_enter; return; fi
  printf "  ${CYAN}testing clone 1${R}\n  pkg : ${WHITE}%s${R}\n  link: ${WHITE}#1 of %s${R}\n\n" "$CLONE1_PKG" "$CLONE1_FILE"
  launch "$CLONE1_PKG" "${TLINKS[0]}"
  printf "  ${GREEN}fired.${R} did clone 1 land in a server?\n  if not, edit option 8 and retest.\n"; pause_enter; }

settings(){ load_config; while true; do settings_screen; read -r s; case "$s" in
  1) pick_pkg && CLONE1_PKG="$PICKED";; 2) printf "  file for clone 1: "; read -r v; [ -n "$v" ]&&CLONE1_FILE="$v";;
  3) pick_pkg && CLONE2_PKG="$PICKED";; 4) printf "  file for clone 2: "; read -r v; [ -n "$v" ]&&CLONE2_FILE="$v";;
  5) printf "  load seconds: "; read -r v; [[ "$v" =~ ^[0-9]+$ ]]&&SETTLE="$v";; 6) printf "  hold seconds: "; read -r v; [[ "$v" =~ ^[0-9]+$ ]]&&HOLD="$v";;
  7) [ "$STAGGER" = 1 ]&&STAGGER=0||STAGGER=1;; 8) edit_tmpl;;
  9) [ "$WAKELOCK" = 1 ]&&WAKELOCK=0||WAKELOCK=1;; a|A) [ "$TERMUXAPI" = 1 ]&&TERMUXAPI=0||TERMUXAPI=1;;
...
refresh_now(){ load_config; fit; clr; prule; center "REFRESH LINKS" "$PINK"; prule; echo
  fetch_file "$CLONE1_FILE"; fetch_file "$CLONE2_FILE"; load_into LINKS1 "$CLONE1_FILE"; load_into LINKS2 "$CLONE2_FILE"
  printf "  ${CYAN}%-14s${R} ${WHITE}%s links${R}\n" "$CLONE1_FILE" "${#LINKS1[@]}"
  printf "  ${CYAN}%-14s${R} ${WHITE}%s links${R}\n\n" "$CLONE2_FILE" "${#LINKS2[@]}"; pause_enter; }
...
clone_lines(){ local slot="$1" pkg="$2" file="$3" idx="$4" n="$5" last="$6" nx="$7" en="$8"
  local sp now secs; sp=$(short_pkg "$pkg"); [ -z "$sp" ]&&sp="(unset)"
  rowc " $slot $sp  $file" "$WHITE"; cstate "$en" "$last"; now=$(date +%s); secs=$((nx-now)); ((secs<0))&&secs=0; ((secs>999))&&secs=999
...
render_run(){ clr; prule
  ...
  prule; clone_lines 1 "$CLONE1_PKG" "$CLONE1_FILE" "$i1" "$n1" "$last1" "$nx1" "$en1"
  clone_lines 2 "$CLONE2_PKG" "$CLONE2_FILE" "$i2" "$n2" "$last2" "$nx2" "$en2"; prule
...
run_loop(){ load_config; fit
  ...
  load_into LINKS1 "$CLONE1_FILE"; load_into LINKS2 "$CLONE2_FILE"; n1=${#LINKS1[@]}; n2=${#LINKS2[@]}
  ...
  while true; do now=$(date +%s)
    if ((paused==0)); then
      if ((en1==1)) && ((now>=nx1)); then launch "$CLONE1_PKG" "${LINKS1[i1-1]}"; last1=$now; save_sess 1 "$i1"
        i1=$((i1+1)); if ((i1>n1)); then i1=1; load_into LINKS1 "$CLONE1_FILE"; n1=${#LINKS1[@]}; ((n1==0))&&en1=0; fi; nx1=$((now+cycle))
        notify "C1 #$i1/$n1  -  C2 #$i2/$n2"; fi
      if ((en2==1)) && ((now>=nx2)); then launch "$CLONE2_PKG" "${LINKS2[i2-1]}"; last2=$now; save_sess 2 "$i2"
        i2=$((i2+1)); if ((i2>n2)); then i2=1; load_into LINKS2 "$CLONE2_FILE"; n2=${#LINKS2[@]}; ((n2==0))&&en2=0; fi; nx2=$((now+cycle))
        notify "C1 #$i1/$n1  -  C2 #$i2/$n2"; fi
    fi
    render_run
