...
#  2 Roblox clones - timed hopping - ONE master links file, sliced by slot
#  repo: github.com/lucivaantarez/rfhop
#  v1.4 - terminal-safe: stty restore (no stuck typing), live resize on
#         rotate, ASCII frame, auto-fit, hard refresh, Ctrl-C stops,
#         optional Termux API
# ============================================================
VERSION="1.4.0"
...
# ---- terminal safety ----
cook(){ [ -t 0 ] && stty sane 2>/dev/null; return 0; }   # restore echo + line mode (kills "stuck typing")

# ---- width auto-fit + ASCII frame ----
INNER=38; RULE=""; EMPTY=""
detect_width(){ local w="${COLUMNS:-}"; [ -z "$w" ] && w=$(tput cols 2>/dev/null)
  [[ "$w" =~ ^[0-9]+$ ]] || w=40; INNER=$((w-2)); ((INNER>40))&&INNER=40; ((INNER<22))&&INNER=22; }
...
cleanup(){ printf '\033[0m'; cook; wakelock_off; notify_clear; }
trap 'cleanup; printf "\n"; exit 0' INT TERM
trap 'fit' WINCH    # re-size the frame live when the screen rotates/resizes

prule(){ printf "%s%s%s\n" "$PINK" "$RULE" "$R"; }
rowc(){ local c="${1:0:INNER}" col="$2" pad; pad=$((INNER-${#c})); ((pad<0))&&pad=0
  printf "%s|%s%s%s%*s%s|%s\n" "$PINK" "$R" "$col" "$c" "$pad" "" "$PINK" "$R"; }
center(){ local t="${1:0:INNER}" col="$2" len pad l r; len=${#t}; pad=$((INNER-len)); ((pad<0))&&pad=0; l=$((pad/2)); r=$((pad-l))
  printf "%s|%s%*s%s%s%s%*s%s|%s\n" "$PINK" "$R" "$l" "" "$col" "$t" "$R" "$r" "" "$PINK" "$R"; }
kv(){ local lw=8 vw; vw=$((INNER-3-lw)); ((vw<1))&&vw=1
  printf "%s|%s   %s%-*s%s%s%-*s%s%s|%s\n" "$PINK" "$R" "$CYAN" "$lw" "${1:0:$lw}" "$R" "$WHITE" "$vw" "${2:0:$vw}" "$R" "$PINK" "$R"; }
menurow(){ local lw; lw=$((INNER-8)); ((lw<1))&&lw=1
...
pause_enter(){ cook; printf "  ${PINK}press enter${R} "; read -r _; }
...
slice_into(){ local -n a="$1"; local slot="$2"; a=(); local start=$(((slot-1)*CHUNK)) k idx
  ((start<0))&&start=0; for ((k=0;k<CHUNK;k++)); do idx=$((start+k)); [ -n "${ALL[idx]:-}" ] && a+=("${ALL[idx]}"); done; }
...
# ===================== screens =====================
home_screen(){ cook; fit; local p1 p2; p1=$(short_pkg "$CLONE1_PKG"); [ -z "$p1" ]&&p1="(unset)"; p2=$(short_pkg "$CLONE2_PKG"); [ -z "$p2" ]&&p2="(unset)"
...
PICKED=""
pick_pkg(){ PICKED=""; detect_clones; cook; fit; clr; prule; center "PICK PACKAGE" "$PINK"; prule; echo
...
edit_tmpl(){ cook; fit; clr; printf "  ${CYAN}current launch template:${R}\n  ${WHITE}%s${R}\n\n  placeholders: ${CYAN}%%PKG%%  %%URL%%${R}\n  new (enter=keep):\n  " "$START_TMPL"; read -r v; [ -n "$v" ] && START_TMPL="$v"; }

test_join(){ load_master; slice_into TLINKS "$CLONE1_SLOT"; cook; fit; clr
...
settings(){ load_config; while true; do settings_screen; read -r s; case "$s" in
  1) pick_pkg && CLONE1_PKG="$PICKED";; 2) cook; printf "  clone 1 slot #: "; read -r v; [[ "$v" =~ ^[0-9]+$ ]]&&CLONE1_SLOT="$v";;
  3) pick_pkg && CLONE2_PKG="$PICKED";; 4) cook; printf "  clone 2 slot #: "; read -r v; [[ "$v" =~ ^[0-9]+$ ]]&&CLONE2_SLOT="$v";;
  5) cook; printf "  master file name: "; read -r v; [ -n "$v" ]&&MASTER="$v";; 6) cook; printf "  links per slot: "; read -r v; [[ "$v" =~ ^[0-9]+$ ]]&&CHUNK="$v";;
  7) cook; printf "  load seconds: "; read -r v; [[ "$v" =~ ^[0-9]+$ ]]&&SETTLE="$v";; 8) cook; printf "  hold seconds: "; read -r v; [[ "$v" =~ ^[0-9]+$ ]]&&HOLD="$v";;
...
refresh_now(){ load_config; cook; fit; clr; prule; center "REFRESH LINKS" "$PINK"; prule; echo
...
run_loop(){ load_config; fit
  ...
  while true; do now=$(date +%s)
    ...
    if read -rsn1 -t 1 k; then case "$k" in
      ...
      q|Q|0) wakelock_off; notify_clear; toast "rfhop stopped"; cook; clr; return;; esac
    fi
  done; }

# ===================== main =====================
cook; load_config
while true; do cook; load_config; home_screen; read -r c; case "$c" in
  1) run_loop; cook;; 2) settings; cook;; 3) refresh_now; cook;;
